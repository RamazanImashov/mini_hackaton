...
def index():
    if request.method == 'POST':
        ...
        # Проверка, есть ли продукт с таким ID
        if product_id:
            product = db.read(product_id)
            if product:
                return f"Ошибка: Продукт с ID {product_id} уже существует"

        # Создание нового продукта
        data = {"id": int(product_id), "name": product_name}
        db.create(data)
